...
BEGIN {
  ...
  if ($ENV{'STC_PRIVATE_INSTALL_DIR'} eq "") { 
    use File::Basename;
    my @fullName = grep /StcPerl.pm/, map {"$INC{$_}\n"} keys %INC;  
...
sub new {
  my $package = shift;
  _checkConfig();  
  use if Config::myconfig() =~ m/usethreads=define/ && $^O !~ m/Win32/, "sPerlmt";
  use if Config::myconfig() =~ m/usethreads=undef/ || scalar($^O =~ m/Win32/), "sPerl";
  sPerlc::salInit();
...
sub create {  
  ...
  my($sv, $retHandle);
  $sv = sPerl::StringVector->new();
  _packArgs($sv, @pairs);
  $retHandle = sPerlc::salCreate($type, $sv);  
  return $retHandle;
}
...
sub get {  
  ...
  my @retV = @{sPerlc::salGet($type, \@svecdashes)};
  if(scalar(@retV) != 1) {
...
sub perform {  
  my($head, $type, @pairs) = @_;    
  my $sv = sPerl::StringVector->new();
  _packArgs($sv, @pairs);
  my @retV = @{sPerlc::salPerform($type, $sv)};  
  if (scalar(@retV) == 1) {

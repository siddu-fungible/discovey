##Library Header
#
# $Id: //TestCenter/4.81_rel/content/HltAPI/SourceCode/parse_dashed_args.tcl#1 $
...
if {[info exists _parse_dashed_argsLibraryAlreadySourced]} {
    return
}
set _parse_dashed_argsLibraryAlreadySourced 1
...
proc ::sth::parse_dashed_args {args} {
    ...
    if {![info exists ::tcl_interactive]} {
        ###introduced to load SpirentHltApi in GUI tclshell
    ...
    if {($mandatory_list != "") && (![regexp {^ +$} $mandatory_list])} {
        set parse_result [::sth::_parse_check_args mandatory $mandatory_list]
    ...
    if {($optional_list != "") && (![regexp {^ +$} $optional_list])} {
        set parse_result [::sth::_parse_check_args optional $optional_list]
    ...
    for { set i 0 } { $i < $num_args } { incr i } {
        ...
        if {[regexp {^-} $arg]} {
            ...
            if {$arg == ""} {
                ...
                return -code error [::sth::_build_parse_error \
            ...
            if {[info exists check_array($arg)] && \
                ...
                while {![regexp {^-} $next_arg] || $next_arg_has_brace} {
                    ...
                    if {[info exists value]} {
                        ...
                        if {[catch {llength $next_arg} errmsg]} {
        ...
        if {[info exist check_array($arg)]} {
            # Check to see if this argument is a FLAG type
            if {[regexp -- {-MULTILIST} $check_array($arg)]} {
                set value "\"$value\""
            }
            if {[regexp -- {-FLAG} $check_array($arg)]} {
                ...
                if {[info exists value]} {
                    if {![string is boolean $value]} {
                        return -code error [::sth::_build_parse_error \
                            $calling_proc_name "Invalid value\
                            \"[join $value]\" for \"-$arg\"\ argument.\n\
                            Valid values are: \"0|1|true|false|yes|no\""]
                    }

                    # When the FLAG has no associated DEFAULT value, we
                    # want to treat "false" values as having not been
                    # specified at all and "true" values as having been
                    # specified only with the switch (no value), for
                    # backwards-compatibility. [CSCeh94732]
            ...
            } elseif {![info exist value] || [regexp {^\s+$} $value]} {
                # Null value for non-flag type, throw error

                return -code error [::sth::_build_parse_error $calling_proc_name \
            ...
            if {$passthru} {
                ...
                return -code error [::sth::_build_parse_error \
                      ...
                      \"-$arg\" not found in mandatory or optional argument\
                       list,it is not a supported switch."]
            }
        } else {
        ...
        if {[info exist check_array($arg)]} {
            ...
            if {[info exists value]} {
                ...
                if { "" == $value } {
                    return -code error "The value of switch -$value SHOULD be $check_array($arg)"
                }
            ...
            foreach element $loop_value {
                foreach check_val $check_array($arg) {
                    ...
                    if {[regexp {^-} $check_val]} {
                        ...
                        switch -regexp -- $check_val {
                            ...
                            ^-IP$ {
                                ...
                                append ipv6 "|($ipv6byte:$ipv6byte:$ipv6byte:"
                                append ipv6 "$ipv6byte\::($ipv6byte:){0,2}"
                                append ipv6 "($ipv6byte)?)"
                                # 5 bytes followed by :: followed by 0-2 bytes
                                append ipv6 "|($ipv6byte:$ipv6byte:$ipv6byte:"
                                append ipv6 "$ipv6byte:$ipv6byte\::($ipv6byte:){0,1}"
                                append ipv6 "($ipv6byte)?)"
                                # 6 bytes followed by :: followed by 0-2 bytes
                                append ipv6 "|($ipv6byte:$ipv6byte:$ipv6byte:"
                                append ipv6 "$ipv6byte:$ipv6byte:$ipv6byte\::"
                                append ipv6 "($ipv6byte)?)"
                                # 7 bytes followed by :: followed by 0 bytes
                                append ipv6 "|($ipv6byte:$ipv6byte:$ipv6byte:"
                                append ipv6 "$ipv6byte:$ipv6byte:$ipv6byte:$ipv6byte\::)"
                                append ipv6 ")"
                                # 1 byte followed by :: followed by 0-6 ipv4 bytes
                                append ipv6 "|($ipv6byte\::($ipv6byte:){0,5}$ipv4)"
                            ...
                            ^-MAC$ {
                                ...
                                #xx:xx:xx:x:xx:xx --> xx:xx:xx:0x:xx:xx
                                #convert the element from sigle byte into double bytes
                                set element [::sth::mac_convert $element]
                                set cscfmt {[0-9a-fA-F][0-9a-fA-F][0-9a-fA-F][0-9a-fA-F]}
                                append cscfmt {[\.\:\-][0-9a-fA-F][0-9a-fA-F][0-9a-fA-F]}
                                append cscfmt {[0-9a-fA-F]}
                                append cscfmt {[\.\:\-][0-9a-fA-F][0-9a-fA-F][0-9a-fA-F]}
                                append cscfmt {[0-9a-fA-F]}
                                set stdfmt {[0-9a-fA-F][0-9a-fA-F][\.\:\-]}
                                append stdfmt {[0-9a-fA-F][0-9a-fA-F][\.\:\-]}
                                append stdfmt {[0-9a-fA-F][0-9a-fA-F][\.\:\-]}
                                append stdfmt {[0-9a-fA-F][0-9a-fA-F][\.\:\-]}
                                append stdfmt {[0-9a-fA-F][0-9a-fA-F][\.\:\-]}
                                append stdfmt {[0-9a-fA-F][0-9a-fA-F]}
                                set check_val "^($stdfmt|$cscfmt)$"
                                set valid_string "MAC address"
                            }
                            ^-RANGE: {
                                # only check the range of the numeric value.
                                if {[regexp "NUMERIC" $check_array($arg)]} {
                                    ...
                                    set valid_string "Number between $low_val and\
                                                     $high_val"
                                } elseif {[regexp "MAC" $check_array($arg)]} {
                                    if {![regexp -- {-RANGE\:(([0-9a-fA-F][\.\:\-]|[0-9a-fA-F])+)\-(([0-9a-fA-F][\.\:\-]|[0-9a-fA-F])+)} \
                                          $check_val - \
                                          low_val low_val1 high_val high_val1]} {
                                        return -code error [::sth::_build_parse_error \
                                                            $calling_proc_name \
                                                            "Invalid check value specification:\
                                                            $arg:\
                                    ...
                                    set valid_string "MAC Addr between $low_val and \
                                    $high_val"
                                } elseif {[regexp "POWER" $check_array($arg)]} {
                                    if {![regexp -- {-RANGE\:([0-9]+)\-([0-9]+)} \
                                              $check_val - \
                                              low_val high_val]} {
                                        return -code error [::sth::_build_parse_error \
                            ...
                            ^-FLAG$ {
                                # Flag types will have already been checked
                                continue
                            }
                            ^-DECIMAL$ {
                                # A decimal value
                            ...
                            ^-NUMERIC$ {
								...
								if {![regexp "$check_val" $element]} {
									catch {set element [expr $element]}
								}
                                set valid_string "A numeric only value"
                            }
                            ^-POWER$ {
                                # A power only value
                                set check_type "POWER"
                            }
                            ^-ALPHA$ {
                                # An alphabetic only value
                                set check_val {^[a-zA-Z]+$}
                                set valid_string "An alphabetic only value"
                            }
                            ^-ALPHANUM$ {
                            ...
                            ^-LIST_SIZE: {
                                ...
                                set check_val ""
                                set valid_string "List size $list_size"
                            }
                            ^-REQUIRES: -
                            ^-EXCLUDES: {
                            ...
                            default {
                                return -code error [::sth::_build_parse_error \
                                           $calling_proc_name \
                                           "Invalid check value specification: Check type\
                    ...
                    if {$check_type == "RANGE"} {
                        ...
                        if {[regexp "NUMERIC" $check_array($arg)]} {
                            ...
                            if {($temp_element < $low_val) || \
                                ...
                                append check_error_str\
                                    "\n[::sth::_build_parse_error $calling_proc_name \
                                      "Invalid $value_type\
                            ...
                            set temp_low [string tolower [regsub -all {[\.]|[\:]|[\-]} $low_val ""]]
                            set temp_high [string tolower [regsub -all {[\.]|[\:]|[\-]} $high_val ""]]
                            set temp_element [string tolower [regsub -all {[\.]|[\:]|[\-]} $element ""]]
                            if {($temp_element < $temp_low) || \
                                 ($temp_element > $temp_high)} {
                                append check_error_str\
                                "\n[::sth::_build_parse_error $calling_proc_name \
                                    "Invalid $value_type\
                                    \"$element\" for \"-$arg\"\ argument.\n\
                    ...
                    } elseif {$check_type == "POWER"} {
                        if {[expr $element&[expr $element-1]]} {
                            append check_error_str\
                                "\n[::sth::_build_parse_error $calling_proc_name \
                                  "Invalid $value_type\
                                \"$loop_value\" for \"-$arg\"\ argument.\n\
                                Valid values are: \"$valid_string\""]"
                            break
                        } else {
                            set check_error_str ""
                        ...
                        if {![regexp "$check_val" $element]} {
                            if {[regexp {set\|seq\|confed_set\|confed_seq} $check_val]} {
                                set valid_string "as_seq:1 or as_seq:4.1 or as_seq:4:3 or as_seq:4:3,1.2,5"
                            }
                            append check_error_str\
                                "\n[::sth::_build_parse_error $calling_proc_name \
                                  "Invalid $value_type\
    ...
    if {$mandatory_arg_count && \
       ($mandatory_arg_count_actual != $mandatory_arg_count)} {
        #handle $mandatory_string by adding "-" before each argument
        set mandatory_string_update ""
        set mandatory_string_actual_update ""
        foreach arg $mandatory_string {
            lappend mandatory_string_update -$arg
        }
        foreach arg $mandatory_string_actual {
            lappend mandatory_string_actual_update -$arg
        }
        return -code error [::sth::_build_parse_error $calling_proc_name \
              "Missing mandatory arguments\n\
              Expected: $mandatory_string_update\n\
              Actual: $mandatory_string_actual_update"]
    }
...
#
# Synopsis:
#   _parse_check_args <argtype> <arglist>
#
# Arguments:
...
proc ::sth::_parse_check_args {argtype arglist} {
        ...
        foreach line $lines {
            ...
            if {$type == "DEFAULT"} {
                ...
                switch -exact -- $type {
                    ...
                    default {
                        return -code error "\n$procName: Invalid check value specification:\
                            Check type \"$type\" is unknown."
                    }
                }
            }
            # Set return keys
...
proc ::sth::_build_parse_error {calling_proc_name msg} {
...
proc ::sth::mac_convert {element} {
    foreach sym [list . : -] {
        if {[llength [split $element $sym]] == 6} {
            break;    
        }    
    }
    set newelement ""
    foreach byte [split $element $sym] {
        if {[string length $byte] < 2} {
            set byte "0$byte"
        }
        set newelement [concat $newelement $byte]
    }
    return [join $newelement $sym]
}

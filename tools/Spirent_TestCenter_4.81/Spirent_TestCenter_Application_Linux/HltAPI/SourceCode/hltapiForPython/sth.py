...
def _pydict2tclstr(handle_ret_py, dict_ret_py, key_str):
    ...
    if re.match('^py_port', handle_ret_py, flags=re.I) is not None:
        tcl_str = 'set {} {}'.format(handle_ret, dict_ret_py)  + '\n'
    else:
...
def _hlt_params_conv(**arg):
    ...
    for key in list(arg.keys()):
        ...
        elif sys.version_info < (3,0) and type(arg[key]) is unicode:
            # Robot scripts pass unicode strings which are not str in Python 2.
            arg[key] = str(arg[key])
...
def connect(**arg):
    ...
    # fixed US36834 : if get following error message, will return the error massage 
    # {log {portlist parameters are unmatched in the loaded config file, the number of unmatched port is XX }} {status 0}
    # error message formate is different from 
    # {offline 0} {port_handle {{10 {{61 {{44 {{2 {{6/8 port1}}}}}}}}}}} {status 1}

    if (re.match(r'.*?{log {(.*?[^}]){status 0}}} ', ret,re.I)):
        retnew['status'] = '0'
        retnew['log'] = re.match(r'.*?{log {(.*?[^}]){status 0}}} ', ret,re.I).group(1)
        return retnew
...
def emulation_6pe_6vpe_provider_port_config(**arg):
    return _process_cmd("emulation_6pe_6vpe_provider_port_config",**arg)

def emulation_6pe_6vpe_cust_port_config(**arg):
    return _process_cmd("emulation_6pe_6vpe_cust_port_config",**arg)

def emulation_6pe_6vpe_config(**arg):
    return _process_cmd("emulation_6pe_6vpe_config",**arg)

def emulation_6pe_6vpe_control(**arg):
    return _process_cmd("emulation_6pe_6vpe_control",**arg)

def emulation_6pe_6vpe_info(**arg):
    return _process_cmd("emulation_6pe_6vpe_info",**arg)
...
def emulation_openflow_switch_config(**arg):
    return _process_cmd("emulation_openflow_switch_config",**arg)

def emulation_openflow_switch_control(**arg):
    return _process_cmd("emulation_openflow_switch_control",**arg)

def emulation_openflow_switch_stats(**arg):
    return _process_cmd("emulation_openflow_switch_stats",**arg)
...
def reserve_ports(**arg):
    return _process_cmd("reserve_ports",**arg)
	
def start_test(**arg):
    return _process_cmd("start_test",**arg)

def emulation_synce_config(**arg):
    return _process_cmd("emulation_synce_config",**arg)

def emulation_synce_control(**arg):
    return _process_cmd("emulation_synce_control",**arg)

def emulation_synce_stats(**arg):
    return _process_cmd("emulation_synce_stats",**arg)
    
def rfc2544_asymmetric_config(**arg):
    return _process_cmd("rfc2544_asymmetric_config",**arg)

def rfc2544_asymmetric_control(**arg):
    return _process_cmd("rfc2544_asymmetric_control",**arg)

def rfc2544_asymmetric_stats(**arg):
    return _process_cmd("rfc2544_asymmetric_stats",**arg)
    
def rfc2544_asymmetric_profile(**arg):
    return _process_cmd("rfc2544_asymmetric_profile",**arg)
	
def emulation_mcast_wizard_config(**arg):
    return _process_cmd("emulation_mcast_wizard_config",**arg)
    
def sequencer_config(**arg):
    return _hltpy_sequencer_config(**arg)
    
def emulation_vxlan_evpn_overlay_control(**arg):
    return _process_cmd("emulation_vxlan_evpn_overlay_control",**arg)

def emulation_vxlan_evpn_overlay_stats(**arg):
    return _process_cmd("emulation_vxlan_evpn_overlay_stats",**arg)

def emulation_vxlan_evpn_overlay_port_config(**arg):
    return _process_cmd("emulation_vxlan_evpn_overlay_port_config",**arg)

def emulation_nonvxlan_evpn_overlay_port_config(**arg):
    return _process_cmd("emulation_nonvxlan_evpn_overlay_port_config",**arg)

def emulation_vxlan_evpn_overlay_wizard_config(**arg):
    return _process_cmd("emulation_vxlan_evpn_overlay_wizard_config",**arg)

def emulation_mpls_ip_vpn_provider_port_config(**arg):
    return _process_cmd("emulation_mpls_ip_vpn_provider_port_config",**arg)

def emulation_mpls_ip_vpn_cust_port_config(**arg):
    return _process_cmd("emulation_mpls_ip_vpn_cust_port_config",**arg)

def emulation_mpls_ip_vpn_config(**arg):
    return _process_cmd("emulation_mpls_ip_vpn_config",**arg)

def emulation_mpls_ip_vpn_control(**arg):
    return _process_cmd("emulation_mpls_ip_vpn_control",**arg)

def emulation_mpls_ip_vpn_info(**arg):
    return _process_cmd("emulation_mpls_ip_vpn_info",**arg)

def emulation_lsp_ping_config(**arg):
    return _process_cmd("emulation_lsp_ping_config",**arg)

def emulation_lsp_ping_message_config(**arg):
    return _process_cmd("emulation_lsp_ping_message_config",**arg)

def emulation_lsp_ping_fec_config(**arg):
    return _process_cmd("emulation_lsp_ping_fec_config",**arg)

def emulation_mplstp_y1731_oam_control(**arg):
    return _process_cmd("emulation_mplstp_y1731_oam_control",**arg)

def emulation_mplstp_y1731_oam_stats(**arg):
    return _process_cmd("emulation_mplstp_y1731_oam_stats",**arg)

def emulation_micro_bfd_config(**arg):
    return _process_cmd("emulation_micro_bfd_config",**arg)

def emulation_micro_bfd_control(**arg):
    return _process_cmd("emulation_micro_bfd_control",**arg)

def emulation_micro_bfd_info(**arg):
    return _process_cmd("emulation_micro_bfd_info",**arg)

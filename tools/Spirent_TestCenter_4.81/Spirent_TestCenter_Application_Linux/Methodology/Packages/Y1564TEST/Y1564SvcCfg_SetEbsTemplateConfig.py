...
def set_ebs_color_aware_schedule(tag_name, b, params):
    ...
    # Sometimes the gcd does wonky things like getting a 5.000e-14,
    # in which case we revert to a ratio
    if gcd < 1:
        big = max(cir_load, eir_load)
        small = min(cir_load, eir_load)
        gcd = round(big / small, 4)
    green_burst = int(round(cir_load / gcd))
    yellow_burst = int(round(eir_load / gcd))

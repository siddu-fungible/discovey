from StcIntPythonPL import CStcSystem, PLLogger, CHandleRegistry
import json
...
def print_stm_rt_res_objects(tagname, tagged_objects, params):
    plLogger = PLLogger.GetLogger("STM")
    stc_sys = CStcSystem.Instance()
    meth_mgr = stc_sys.GetObject("StmMethodologyManager")
    rt_res_list = meth_mgr.GetObjects("StmRtResult")
    for rt_res in rt_res_list:
        plLogger.LogDebug("rt_res: " + rt_res.Get("Name"))
        plLogger.LogDebug("ResultInfoJson: " + rt_res.Get("ResultInfoJson"))
        plLogger.LogDebug("ResultDisplayJson: " +
                          rt_res.Get("ResultDisplayJson"))
        plLogger.LogDebug("ResultDataJson: " + rt_res.Get("ResultDataJson"))
        plLogger.LogDebug("---------------------")
...
def get_controller():
    stc_sys = CStcSystem.Instance()
    return stc_sys.GetObject('ClusterController')


def get_session():
    ctlr = get_controller()
    if not ctlr:
        return None, 'No cluster controller found.'
...
def get_ms_agent_stats(tagname, tagged_objects, params):
    ...
    sess, err_str = cmd_utils.get_session()
    if err_str:
        return err_str
    token = sess.Get("SecurityToken")
    db = "metrics"
    event_table = "CLOUDSTRESS_BENCHMARK_FIXEDCOUNT_cs_bll_events"
...
# }
def config_specific_node_inst_cpu_load(tagname, tagged_objects, params):
    plLogger = PLLogger.GetLogger("methodology")
    plLogger.LogDebug("params: " + str(params))
    err_str, j_dict = json_utils.load_json(params)
    if err_str:
    ...
    for prop in ["MinCpuLoad", "MaxCpuLoad", "TemplateId"]:
    ...
    with AutoCommand(PKG_CS + ".ConfigCpuLoadParamsCommand") as cmd:
        ...
        cmd.Set("MinCpuLoad", j_dict["MinCpuLoad"])
        cmd.Set("MaxCpuLoad", j_dict["MaxCpuLoad"])
        cmd.Execute()
...
#     "NodeId": "Node discovery ID; can be empty string"
# }
def config_specific_node_inst_mem_load(tagname, tagged_objects, params):
    plLogger = PLLogger.GetLogger("methodology")
    plLogger.LogDebug("params: " + str(params))
    err_str, j_dict = json_utils.load_json(params)
    if err_str:
    ...
    for prop in ["MinReadRate", "MaxReadRate", "MinWriteRate",
                 "MaxWriteRate", "ReadSize", "WriteSize",
    ...
    with AutoCommand(PKG_CS + ".ConfigMemoryLoadParamsCommand") as cmd:
        cmd.SetCollection("InstanceList", inst_hnd_list)
        cmd.SetCollection("NodeList", node_hnd_list)
        cmd.Set("MinReadRate", j_dict["MinReadRate"])
        cmd.Set("MaxReadRate", j_dict["MaxReadRate"])
        cmd.Set("MinWriteRate", j_dict["MinWriteRate"])
        cmd.Set("MaxWriteRate", j_dict["MaxWriteRate"])
...
#     "NodeId": "Node discovery ID; can be empty string"
# }
def config_specific_node_inst_net_load(tagname, tagged_objects, params):
    plLogger = PLLogger.GetLogger("methodology")
    plLogger.LogDebug("params: " + str(params))
    err_str, j_dict = json_utils.load_json(params)
    if err_str:
...
#     "NodeId": "Node discovery ID; can be empty string"
# }
def config_specific_node_inst_stor_load(tagname, tagged_objects, params):
    plLogger = PLLogger.GetLogger("methodology")
    plLogger.LogDebug("params: " + str(params))
    err_str, j_dict = json_utils.load_json(params)
    if err_str:
    ...
    for prop in ["MinReadRate", "MaxReadRate", "MinWriteRate",
                 "MaxWriteRate", "ReadSize", "WriteSize",
                 "TargetDevice", "EnableFileSize", "FileSize", "AccessPattern",
                 "QueueDepth", "TemplateId"]:
        if prop not in j_dict:
            return prop + " must be specified in input params"
    ...
    # Configure the new Storage Load
    with AutoCommand(PKG_CS + ".ConfigStorageLoadParamsCommand") as cmd:
        ...
        cmd.Set("ReadSize", j_dict["ReadSize"])
        cmd.Set("WriteSize", j_dict["WriteSize"])
        cmd.Set("TargetDevice", j_dict["TargetDevice"])
        cmd.Set("EnableFileSize", j_dict["EnableFileSize"])
...
def get_tdigest_agent_stats(tagname, tagged_objects, params):
    ...
    return result_utils.get_tdigest_agent_stats(iteration, thr_stat, tdigest_stat)

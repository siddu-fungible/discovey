    ...
    def _connect(self):
        ...
        try:
            ...
            while elapsed_time < self.connect_retry_timeout_max:
                if attempt:
                    print("Attempting connect: %d" % (attempt))
                ...
                while current_loop_count < loop_count_max:
                    try:
                        ...
                        if i == 0:
                            ...
                            self._debug_expect_buffer()
                            print("Connected to %s Using %s:%s" % (self.host_ip,
                                                                             self.ssh_username,
                                                                             self.ssh_password))
                            connected = True
                            self.command("")
                        ...
                        attempt += 1
                        retry_sleep = 1
                        print("Sleeping {} seconds for next attempt".format(retry_sleep))
        ...
        try:
            ...
            if not self.handle:
                if not self._connect():
                    raise Exception("Unable to connect to %s, username: %s, password: %s %d" % (self.host_ip,
                                                                                             self.ssh_username,

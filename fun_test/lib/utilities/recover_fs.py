...
if __name__ == "__main__":
    ...
    for fs_name in fs_names:
        try:
            ...
            for f1_inex in range(2):
                bmc.setup_serial_proxy_connection(f1_index=f1_inex)
                bmc.reset_f1(f1_index=f1_inex)
                preamble = bmc.get_preamble(f1_index=f1_inex)
                bmc.validate_u_boot_version(output=preamble, minimum_date=Fs.MIN_U_BOOT_DATE)
                bmc.u_boot_load_image(index=f1_inex, tftp_image_path=stable_image_gz, boot_args=boot_args, mpg_ips=["10.1.105.154", "10.1.105.153"], gateway_ip="10.1.105.1")

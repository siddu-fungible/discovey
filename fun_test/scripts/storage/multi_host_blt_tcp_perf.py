...
class MultiHostVolumePerformanceScript(FunTestScript):
    ...
    def setup(self):
        ...
        host_status = ensure_all_hosts_up(host_handles=self.host_handles, reboot_timeout=self.reboot_timeout,
                                          load_modules=self.load_modules)
        fun_test.test_assert(host_status, "Ensure all hosts are up")

        host_f1_ping = ensure_host_f1_connectivity(funcp_spec=self.funcp_spec, host_handles=self.host_handles,
                                                   f1_ips=self.f1_ips)
        fun_test.test_assert(host_f1_ping, "Ensure connectivity from hosts to F1's")

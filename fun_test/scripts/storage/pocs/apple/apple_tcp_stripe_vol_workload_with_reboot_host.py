...
class StripeVolHostRebootTestScript(FunTestScript):
    def describe(self):
...
class StripeVolHostRebootTestCase(FunTestCase):
    def describe(self):
        pass
    ...
    def setup(self):
        ...
        if "blt" not in fun_test.shared_variables or not fun_test.shared_variables["blt"]["setup_created"]:
            ...
            if self.reboot_host:
                fun_test.sleep("rebooting host server in", self.reboot_after)
                for index, host_name in enumerate(self.host_info):
                    host_handle = self.host_info[host_name]["handle"]
                    host_handle.reboot(max_wait_time=self.max_wait_time)
                ...
                for index, host_name in enumerate(self.host_info):
                    ...
                    if hasattr(self, "create_file_system") and self.create_file_system:
                        ...
                        fun_test.log("Creating a testfile on XFS volume")
                        warmup_thread_id[index] = fun_test.execute_thread_after(
                            time_in_seconds=wait_time, func=fio_parser, arg1=host_clone[host_name],
                            host_index=index,
                            filename="/mnt/testfile.dat", **self.warm_up_fio_cmd_args)
                    else:
                        warmup_thread_id[index] = fun_test.execute_thread_after(
...
class StripedVolHostReboot(StripeVolHostRebootTestCase):
    def describe(self):
        self.set_test_details(
            id=1,
            summary="Data integrity validation after Host reboot during Sequential write",
            steps='''
                1. Create Stripe volume
                2. Attach volume to one host and perform sequential write
                3. During Sequential write Reboot host
                4. Once host is up, load nvme modules, Perform nvme-connect and re-initiate sequential write
                4. Start Random Read with data integrity
                ''')

    def setup(self):
        super(StripedVolHostReboot, self).setup()

    def run(self):
        super(StripedVolHostReboot, self).run()

    def cleanup(self):
        super(StripedVolHostReboot, self).cleanup()


if __name__ == "__main__":
    testscript = StripeVolHostRebootTestScript()
    testscript.add_test_case(StripedVolHostReboot())
    testscript.run()

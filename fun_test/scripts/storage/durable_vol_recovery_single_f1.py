...
class C16229(DurableVolumeTestcase):
    def describe(self):
        self.set_test_details(id=1,
                              summary="Plex failures(one after the other) upto m in k+m - data at rest",
                              test_rail_case_ids=["C16229"],
                              steps="""
        ...
        7. After Write is completed, fail one of the plex.
        8. Perform read operation now using the data file for pattern match
        """)


class C16230(DurableVolumeTestcase):
    def describe(self):
        self.set_test_details(id=2,
                              summary="Plex failures(one after the other) upto m+1 in k+m - data at rest",
                              test_rail_case_ids=["C16230"],
                              steps="""
        1. Bring up F1 in FS1600
        2. Reboot network connected host and ensure connectivity with F1        
        3. Configure a LSV (on 4:2 EC volume2 on top of the 6 BLT volumes) for actual test        
...
class C18748(DurableVolumeTestcase):
    def describe(self):
        self.set_test_details(id=3,
                              summary="m concurrent plex failures in k+m - data at rest",
                              test_rail_case_ids=["C18748"],
                              steps="""
        1. Bring up F1 in FS1600
        2. Reboot network connected host and ensure connectivity with F1        
        3. Configure a LSV (on 4:2 EC volume2 on top of the 6 BLT volumes) for actual test        

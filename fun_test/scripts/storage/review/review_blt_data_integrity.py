...
class SharedVolumePerfTest(FunTestCase):
    ...
    def setup(self):
        ...
        for index, host in enumerate(self.hosts):
            pattern = utils.generate_uuid()
            fun_test.log("pattern used for fio is")
            fun_test.log(pattern[0:4])
            host.pattern = []
            fio_pattern = '0xff%o\\"'+pattern[0:4]+'\\"-12'
            host.pattern.append(pattern[0:4])
            fun_test.shared_variables["host_pattern"] = host.pattern
            host.instance.sudo_command("dmesg -C")  # cleaning the dmesg output of the previous runs
            fun_test.log("Write,read and verify the data, this might take long time depending on fio --size "
                         "provided")
            fio_output[index] = {}
            ...
            if "multiple_jobs" in self.warm_up_fio_cmd_args:
                ...
                verify_pattern_string = " --verify_pattern={}".format(fio_pattern)
                warm_up_fio_cmd_args["multiple_jobs"] = self.warm_up_fio_cmd_args["multiple_jobs"] + \
                                                        fio_cpus_allowed_args + offset + size + verify_pattern_string + \
                                                        jobs
                fun_test.log("fio command ")
                fun_test.log(warm_up_fio_cmd_args["multiple_jobs"])
...
class ConfigPersistenceAfterReset(FunTestCase):
    ...
    def setup(self):
        ...
        #data integrity check before reboot
        ...
        fun_test.shared_variables["fio"] = {}
        for index, host in enumerate(self.hosts):
            fio_pattern = '0xff%o\\"' + host.pattern[0] + '\\"-12'
            host.instance.sudo_command("dmesg -C")  # cleaning the dmesg output of the previous runs
            fun_test.log("Data integrity check before reboot")
            fio_output[index] = {}
            end_host_thread[index] = host.instance.clone()
            wait_time = len(self.hosts) - index
            if "multiple_jobs" in self.warm_up_fio_cmd_args:
        ...
        try:
            for i, host in enumerate(self.hosts):
                ...
                fun_test.log(fun_test.shared_variables["fio"][i])
                fun_test.test_assert(fun_test.shared_variables["fio"][i],
                                     "Data integrity check before reboot{}".format(host.name))
                fio_output[i] = fun_test.shared_variables["fio"][i]
        except Exception as ex:
            fun_test.critical(str(ex))
            fun_test.log("FIO Command Output from host {}:\n {}".format(host.name, fio_output[i]))
                ...
                for host in self.hosts:
                    ...
                    if nvme_device_list_after_reboot["status"]:
                        fun_test.test_assert_expected(
                            ...
                            message="Expected number of NVMe devices available after reboot at host {}".format(
                                host.name))
                        res = sorted(host.nvme_block_device_list) == sorted(
                            nvme_device_list_after_reboot["nvme_device"])
                        fun_test.test_assert(res, "nvme device names are valid {}".format(
                            nvme_device_list_after_reboot["nvme_device"]))
                break

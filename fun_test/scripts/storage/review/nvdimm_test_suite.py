...
class NvdimmTest1(FunTestCase):
    ...
    def run(self):
        ...
        topology_helper.set_dut_parameters(dut_index=0, custom_boot_args="app=load_mods nvdimm_arm_backup=1 nvdimm_restore=1 demo-mode=backup syslog=6 --test-exit-fast --dpc-uart --dpc-server --csr-replay --all_100g")
      ...
      #  output = bmc_handle.command('gpiotool --set-data-low 208; sleep 1; gpiotool --set-data-high 208') #F1_0, for F1_1, use pin 209
        bmc_host_ip = bmc_handle.host_ip;
        output = bmc_handle.command('ipmitool -I lanplus -H {} -U admin -P admin sensor thresh "F1_0" upper 35 40 45'.format(bmc_host_ip))

...
class RunStorageApiCommands(FunTestCase):
    ...
    def run(self):
        fs_obj_list = [self.topology.get_dut_instance(index=dut_index)
                       for dut_index in self.topology.get_available_duts().keys()]
        storage_controller = fs_obj_list[0].get_storage_controller()
        topology_result2 = storage_controller.topology_api.get_hierarchical_topology()
        fun_test.log(topology_result2)
        fun_test.test_assert(topology_result2 != self.topology_result,
                             message="Comparing topology details after drive format")

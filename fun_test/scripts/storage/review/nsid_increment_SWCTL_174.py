...
class NsidIncrement(FunTestCase):
    def describe(self):
    ...
    def run(self):
        ...
        for i in range(len(self.vol_uuid_list)):
            if i % 2 == 0:
                host_nqn = "nqn.2015-09.com.Fungible:{}".format(host_ip_list[0])
            else:
                host_nqn = "nqn.2015-09.com.Fungible:{}".format(host_ip_list[1])
            vol_uuid = self.vol_uuid_list[i]
            attach_vol_result = self.attach_volume(self.fs_obj_list[0], vol_uuid, host_obj, host_nqn,
                                                   validate_nvme_connect=False, raw_api_call=True)
            fun_test.test_assert(expression=attach_vol_result[0]['status'], message="Attach Volume Successful")
...
if __name__ == "__main__":
    ...
    setup_bringup.add_test_case(NsidIncrement())
    setup_bringup.run()

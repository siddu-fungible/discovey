...
def post_raw_vol_perf_results(log_time, num_ssd, num_volumes, block_size, io_depth, size, operation, write_iops,
                              read_iops, write_bw, read_bw, write_latency, write_90_latency, write_95_latency,
                              write_99_latency,write_99_99_latency, read_latency, read_90_latency,
                              read_95_latency, read_99_latency,read_99_99_latency, num_dpu=-1, num_hosts=-1,
                              shared_volume=False, compression=False,encryption=False,
...
def check_host_f1_connectivity(funcp_spec, host_handle, f1_ips, interface="enp216s0", interface_bringup_timeout=120):
    ...
    try:
        ...
        fun_test.simple_assert(ip_assigned, "Ensure ip is assigned to interface {} on host {}".
                               format(interface, host_handle.host_ip))
    except Exception as ex:
        fun_test.critical(str(ex))
...
def get_drive_uuid_from_device_id(storage_controller, drive_ids_list):
    ...
    fetch_drives = storage_controller.peek(props_tree)

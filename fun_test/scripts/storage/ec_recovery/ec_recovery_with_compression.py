...
def format_all_drives(storage_controller):
    num_drives = 11
    for i in range(0, num_drives):
        label = utils.generate_uuid()
        storage_controller.format_drive(i, label)


def check_dpcserver_running(come_obj):
    dpc_server_ports = [42220, 42221]
    for port in dpc_server_ports:
        out = come_obj.command("ps -aef | grep 'tcp_proxy={}' | grep -vE 'grep|sudo'".format(port))
        if not out:
...
class ECBlockRecoveryScript(FunTestScript):
    ...
    def setup(self):
        ...
        fun_test.shared_variables["space_savings"] = self.space_savings
...
class RecoveryWithFailures(FunTestCase):
    ...
    def setup(self):
        ...
        self.space_savings = fun_test.shared_variables["space_savings"]

        # TODO: Check if dpc server is running if not then start it (for already_deployed=true)
        check_dpcserver_running(self.come_obj[0])
    ...
    def run(self):
        ...
        for index, host_name in enumerate(self.host_info):
            ...
            # Making nvme subsystem string unique
            self.nvme_subsystem = str(self.nvme_subsystem) + ":" + str(self.ctrlr_uuid[-1])
        ...
        for combo in self.ec_list:
            ...
            if self.plex_failure_combination == "all":
                ...
                plex_fail_comb_list.append(tuple(random.sample(range(self.ec_info["ndata"] + self.ec_info["nparity"]),
                                                               self.ec_info["nparity"])))
            for plex_fail_pattern in plex_fail_comb_list:
                ...
                for host_name in self.host_info:
                    ...
                    # Get LSV stats before writing
                    self.lsv_stats_before_write = self.storage_controller.peek(
                        props_tree="storage/volumes/VOL_TYPE_BLK_LSV/{}/stats".format(
                            self.ec_info["uuids"][num]["lsv"][0]), legacy=False, chunk=8192,
                        command_duration=self.command_timeout)
                    ...
                    # Pass compression savings factor to fio so that it writes compressible data accordingly
                    self.fio_write_cmd_args["buffer_compress_percentage"] = self.space_savings
                    self.fio_read_cmd_args["buffer_compress_percentage"] = \
                        self.fio_write_cmd_args["buffer_compress_percentage"]
                    ...
                    if hasattr(self, "m_concurrent_failure") and \
                            self.m_concurrent_failure and not hasattr(self, "m_plus_concurrent_failure"):
                        fio_output = host_handle.pcie_fio(filename=self.host_info[host_name]["fio_filename"],
                                                          cpus_allowed=self.host_info[host_name]["host_numa_cpus"],
                                                          **self.fio_read_cmd_args)
                        fun_test.log("FIO Command Output:\n{}".format(fio_output))
                    ...
                    if (hasattr(self, "mplusfailure") and self.mplusfailure) or \
                            ...
                            (hasattr(self, "k_plus_m_concurrent_failure") and self.k_plus_m_concurrent_failure):
                        plex_to_be_failed = []
                        ...
                        for plex in plex_to_be_failed:
                            if self.plex_fail_method == "ssd_power_off":
                                ...
                                device_fail_status = self.storage_controller.power_toggle_ssd(
                                    "off", device_id=plex, command_duration=self.command_timeout)
                                fun_test.test_assert(device_fail_status["status"],
                                                     "Powering OFF Device ID {}".format(plex))
                                # Validate if Device is marked Failed
                                device_stats = self.storage_controller.get_ssd_power_status(
                    ...
                    for plex in self.device_id_failed:
                        if self.plex_fail_method == "ssd_power_off":
                            ...
                            device_bringup_status = self.storage_controller.enable_device(
                                device_id=plex, command_duration=self.command_timeout)

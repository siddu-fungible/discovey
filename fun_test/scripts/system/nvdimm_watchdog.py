...
class NvdimmTest1(FunTestCase):
    ...
    def run(self):
        ...
        topology_helper.set_dut_parameters(dut_index=0, custom_boot_args="app=load_mods nvdimm_arm_backup=1 nvdimm_restore=1  --serial --dpc-server --dpc-uart --all_100g workload=storage")
        ...
        fun_test.log("causing PC stoppage which should trigger loss of watchdog toggles ...")
        B = board_helper( fs.spec )
        B.fault_stop_heartbeat()
        fun_test.log("wait for watchdog (expect save) ...")
        time.sleep(120)
        fun_test.log("done wait ...")

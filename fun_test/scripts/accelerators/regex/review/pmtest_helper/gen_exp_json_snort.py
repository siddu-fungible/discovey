...
for pat, plds  in pat_pld_files.items():
    ...
    for idx, pld in enumerate(plds):
        ...
        #    print "Excludig ", pld
        pld_base = str(idx+1)
        #pld_base = re.search("test_\d+_(\d+).in", pld).group(1)  # Payload ordering maitained according to test_1_*.in
        ...
        if random_gather:
            ...
            #while gather_lths[0] < 3:
            #     print "First gather entry is < 3, trying random create again ...", gather_lths, pld
            #     gather_lths = random_sum_to(pld_len, n_gather_entries)
        else:

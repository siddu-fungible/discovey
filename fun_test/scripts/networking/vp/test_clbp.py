    ...
    try:
        ...
        for key, val in per_vppkt_output_dict.iteritems():
            ...
            if flow_direction == NuConfigManager.FLOW_DIRECTION_HU_FPG:
                ...
                total_vp_out_etp = int(val[VP_PACKETS_OUT_NU_ETP])
                if VP_PACKETS_OUT_NU_ETP in old_per_vppkt_output_dict[key]:
                    total_vp_out_etp = int(val[VP_PACKETS_OUT_NU_ETP]) - int(old_per_vppkt_output_dict[key][VP_PACKETS_OUT_NU_ETP])
            else:
            ...
            if flow_direction == NuConfigManager.FLOW_DIRECTION_FPG_HU:
                ...
                fun_test.test_assert(compare_spray_values(reference_value, total_vp_out_etp),
                                     "Check counter for %s. Expected value: %s Actual value %s for key %s " %
                                     (VP_PACKETS_OUT_NU_ETP, reference_value, total_vp_out_etp, key))
        result = True
...
class CLBP_NU_HNU(FunTestCase):
    ...
    def run(self):
        ...
        if dut_config['enable_dpcsh']:
            ...
            counter_check = check_per_vp_pkt_spray(flow_direction=self.flow_direction, per_vppkt_output_dict=vp_pkts_stats_2,
                                                   old_per_vppkt_output_dict=vp_pkts_stats_1,

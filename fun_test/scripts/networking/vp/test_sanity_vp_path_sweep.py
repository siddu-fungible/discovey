...
class SpirentSetup(FunTestScript):
    ...
    def setup(self):
        global template_obj, port_1, port_2, interface_1_obj, interface_2_obj, gen_config_obj, \
            ...
            ol_ipv4_l3_config, ol_ipv6_l3_config, destination_mac, flow_direction, fps, nu_config_obj
...
class OverlayIpv4TCP(VPPathIPv4UDP):
    streamblock_obj_1 = None
    min_frame_size = overlay_ipv4_min_frame_length
    update_header = UDP()
    check_analyzer_results = True

    def describe(self):
        self.set_test_details(id=9,
                              summary="Test VP path from %s for VxLAN overlay packet with IPv4 and TCP "
                                      "with frame size incrementing "
                                      "from 108B to 2068" % (self.flow_direction),
                              steps="""
                            1. Create overlay stream on spirent and start traffic in incremental
                            2. Compare Tx and Rx results for frame count
                            3. Check for error counters. there must be no error counter
    ...
    def setup(self):
        ...
        self.streamblock_obj_1.FrameLengthMode = self.streamblock_obj_1.FRAME_LENGTH_MODE_INCR
        self.streamblock_obj_1.InsertSig = False
        self.streamblock_obj_1.MinFrameLength = 108
        self.streamblock_obj_1.MaxFrameLength = 2068

...
class SpirentSetup(FunTestScript):
    ...
    def setup(self):
        ...
        template_obj = SpirentEthernetTrafficTemplate(session_name="vp-sanity", spirent_config=spirent_config,
...
class VPPathIPv4TCP(FunTestCase):
    ...
    def run(self):
        ...
        fun_test.sleep("Letting rx to take place", seconds=5)
...
class VPPathIPv4UDP(VPPathIPv4TCP):
    ...
    def describe(self):
        self.set_test_details(id=2,
                              summary="Test VP path from FPG-HU for IPv4 with UDP with frame size "
                                      "incrementing from 78B to %s" % max_frame_length,
                              steps="""
...
class VPPathIPv6TCP(VPPathIPv4TCP):
    ...
    def describe(self):
        self.set_test_details(id=3,
                              summary="Test VP path from FPG-HU for IPv6 with TCP with frame size incrementing "
                                      "from 78B to %s" % max_frame_length,
                              steps="""
...
class VPPathIPv6UDP(VPPathIPv6TCP):
    ...
    def describe(self):
        self.set_test_details(id=4,
                              summary="Test VP path from FPG-HU for IPv4 with UDP with frame size "
                                      "incrementing from 98B to %s" % max_frame_length,
                              steps="""
...
class OverlayIpv4UDP(VPPathIPv4UDP):
    ...
    def describe(self):
        self.set_test_details(id=5,
                              summary="Test VP path from FPG-HU for VxLAN overlay packet with IPv4 and UDP "

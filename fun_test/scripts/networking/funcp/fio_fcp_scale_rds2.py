...
class RDSVolumePerformanceScript(FunTestScript):
    ...
    def setup(self):
        ...
        if "nvme_tcp_ctrl_fs" in job_inputs:
            fun_test.shared_variables["nvme_tcp_ctrl_fs"] = job_inputs["nvme_tcp_ctrl_fs"].split(",")
        else:
            fun_test.shared_variables["nvme_tcp_ctrl_fs"] = ["fs-67 f11"]
        ...
        for storage_fs in fun_test.shared_variables["storage_fs"]:
            ...
            n = 0
            storage_fs_list = fun_test.shared_variables["storage_fs"]
            for fs in storage_fs_list:
                if fs in fun_test.shared_variables["nvme_tcp_ctrl_fs"]:
                    if 'f10' in fs:
                        nvme_tcp_controller_obj = target_f10_storage_obj[fs]
                    else:
                        nvme_tcp_controller_obj = target_f11_storage_obj[fs]
            ...
            for i in range(0, fun_test.shared_variables["num_vols"]):
                ...
                command_result = target_f10_storage_obj[storage_fs].create_controller(ctrlr_uuid=f10_rds_ctrl[storage_fs],
                                                                          ...
                                                                          nqn='nqn_rds' + str(i),
                                                                          port=ipcfg_port,
                                                                          remote_ip=f11_storage_loop_ip[storage_fs],
                                                                          command_duration=command_timeout)
                fun_test.simple_assert(command_result["status"],
                ...
                command_result = target_f11_storage_obj[storage_fs].create_volume(type="VOL_TYPE_BLK_RDS",
                                                      ...
                                                      remote_nsid=i+1,
                                                      connections=4,
                                                      remote_ip=f10_storage_loop_ip[storage_fs],
                                                      port=ipcfg_port,
                                                      command_duration=command_timeout)

...
def fetch_host_interface_ip(host_name, host_username, host_password):
    hu_ip = None
    try:
...
def ensure_route_exists(linux_obj, network, interface, gateway):
    result = False
    try:
...
def get_network_by_ip(ip, subnet=24):
    return '.'.join(ip.split('.')[:3]) + '.0' + '/%s' % subnet


def get_gateway_by_ip(ip):
    x = ip.split('.')
    x[3] = '1'
    return '.'.join(x)


def get_hu_interface_on_host(linux_obj):
    interface_name = None
    try:
        contents = linux_obj.command(command="ip address")
...
def ensure_same_network(network1, network2):
    return network1 == network2


def get_vlan_ips(come_dict, vlan_interface="vlan1"):
    vlan_list = []
    try:

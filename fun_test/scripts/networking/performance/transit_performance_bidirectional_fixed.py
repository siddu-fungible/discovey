...
TRAFFIC_DURATION = 60
IPV4_FRAMES = [64, 200, 1000, 1500, 3000, 9000, 16380]
...
class NuTransitLatencyIPv4Test(FunTestCase):
    ...
    def describe(self):
        self.set_test_details(id=1,
                              summary="NU Transit Performance Bidirectional Latency Test (IPv4)",
                              steps="""
                              1. Get port handles
    ...
    def run(self):
        ...
        for stream_objs in all_stream_objects:
            ...
            if dut_config['enable_dpcsh']:
                ...
                checkpoint = "Validate FPG FrameCount Tx == Rx for port direction %d --> %d on DUT" % (
                    dut_config['ports'][1], dut_config['ports'][0])
...
class NuTransitJitterTest(FunTestCase):
    ...
    def describe(self):
        self.set_test_details(id=2,
                              summary="NU Transit Performance Bidirectional Jitter Test (IPv4)",
                              steps="""
                              1. Get port handles
                              2. Create stream block EthernetII and IPv4 Headers. Stream Config as follows
                                 a. Frames: %s

...
class SpirentSetup(FunTestScript):
    ...
    def setup(self):
        ...
        destination_ip1 = spirent_config['l3_config']['ipv4']['destination_ip4']
        ...
        for queue in priority_list:
            ...
            if queue == 0:
                fun_test.test_assert(result_dict[fpg_cbfcpause_counters_rx][queue],
                                     message="Ensure CBFCPause_Received is seen for priority %s" % queue)
            else:
                fun_test.test_assert(not result_dict[fpg_cbfcpause_counters_rx][queue],
                                     message="Ensure CBFCPause_Received is not seen for priority %s" % queue)

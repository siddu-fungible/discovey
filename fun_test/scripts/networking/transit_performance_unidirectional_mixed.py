...
class NuTransitPerformance(FunTestScript):
    ...
    def cleanup(self):
        ...
        template_obj.populate_performance_counters_json(test_name="NU Transit Performance Unidirectional Mixed Frames",
                                                        latency_results=latency_results,
                                                        jitter_results=jitter_results,
                                                        file_name="nu_transit_performance_unidirectional_mixed_size")
...
class NuTransitLatencyTest(FunTestCase):
    ...
    def describe(self):
        self.set_test_details(id=1,
                              summary="NU Transit Performance Unidirectional Latency Test (Mixed Size Frames)",
                              steps="""
                              1. Get port handles
    ...
    def run(self):
        ...
        result = template_obj.activate_stream_blocks(stream_obj_list=stream_port_obj_dict[port1])
        fun_test.simple_assert(result, checkpoint)
        ...
        for stream_obj in stream_port_obj_dict[port1]:
            ...
            rate_result = template_obj.validate_traffic_rate_results(
                ...
                stream_objects=[stream_obj], wait_before_fetching_results=False)
...
class NuTransitJitterTest(FunTestCase):
    ...
    def describe(self):
        self.set_test_details(id=2,
                              summary="NU Transit Performance Unidirectional Jitter Test (Mixed Size Frames)",
                              steps="""
                              1. Get port handles
                              2. Create Generator Config for each port and set below parameters
                                 a. Set Duration 60 secs (configurable)
    ...
    def setup(self):
        ...
        checkpoint = "Create Generator Config for %s port" % self.port
        result = template_obj.configure_generator_config(port_handle=self.port,
                                                         generator_config_obj=self.generator_port_obj_dict[self.port])
        fun_test.test_assert(expression=result, message=checkpoint)

...
class SP_Shaper_Q0_SP_EB(FunTestCase):
    ...
    def describe(self):
        self.set_test_details(id=1,
                              summary="To test scheduler when in extra bandwidth zone and both sp and eb is enabled "
                                      "only for Q0",
                              steps="""
        ...
        for port, streams in self.test_streams.iteritems():
            ...
            for stream_details, dscp_val in zip(streams, current_list):
                ...
                set_strict = network_controller_obj.set_qos_scheduler_config(port_num=dut_port_2,
                                                                             ...
                                                                             strict_priority_enable=stream_details[
                                                                                 'sp'],
                                                                             extra_bandwidth=stream_details['eb'])
                fun_test.test_assert(set_strict,
                                     "Ensure strict priority is %s and extra bandwidth is %s set on port %s for queue %s" %
...
class EB_SP_Shaper_DWRR_Non_SP(SP_Shaper_Q0_SP_EB):
    testcase_streamblocks = {}
    streamblock_handles_list = []
    cir = "within_eb_2"
    test_streams = qos_sp_json[cir]
    ...
    def describe(self):
        self.set_test_details(id=2,
                              summary="To test scheduler when in extra bandwidth zone and both sp and eb is "
                                      "enabled only for Q0 and different weights are assigned to queues Q1, Q2 and Q3",
                              steps="""
                              1. Create stream with dscp 0 and dscp 1 on port 1 and with dscp 2 and 3 on port 3
                              2. Ensure strict priority is applied to dscp 0 on egress port and weights of 2,3 and 5 are
                                 assigned for queues Q1, Q2 and Q3 respectively.
                              3. Start traffic
                              4. After 10 seconds, get RxL1BitRate from spirent for each stream
...
class SP_Shaper_All_SP_EB(SP_Shaper_Q0_SP_EB):
    ...
    def describe(self):
        self.set_test_details(id=3,
                              ...
                              steps="""
                              1. Create stream with dscp 0 and dscp 1 on port 1 and with dscp 2 and 3 on port 3
                              2. Ensure strict priority and EB is applied to all queues on egress port.
                              3. Start traffic
                              4. After 10 seconds, get RxL1BitRate from spirent for each stream
                              5. Verify bandwidth is such that queues with lower number hoggs the bandwidth
                              """)


class SP_Shaper_All_SP_Non_EB(SP_Shaper_Q0_SP_EB):
    testcase_streamblocks = {}
    streamblock_handles_list = []
    cir = "within_eb_4"
    test_streams = qos_sp_json[cir]
    ...
    def describe(self):
        self.set_test_details(id=4,
                              summary="Test test scheduler when all queues are within eb and all are SP but "
                                      "having eb flag disabled",
                              steps="""
                              1. Create stream with dscp 0 and dscp 1 on port 1 and with dscp 2 and 3 on port 3
                              2. Ensure strict priority is applied to all queues on egress port.
                              3. Start traffic
                              4. After 10 seconds, get RxL1BitRate from spirent for each stream
                              5. Verify bandwidth is such that it happens in RR as we are EB zone but EB is 
                              disabled for all queues
                              """)


class SP_Shaper_Q0_Q1_EB(SP_Shaper_Q0_SP_EB):
    testcase_streamblocks = {}
    streamblock_handles_list = []
    cir = "within_eb_5"
    test_streams = qos_sp_json[cir]
    ...
    def describe(self):
        self.set_test_details(id=5,
                              summary="To test scheduler when in extra bandwidth zone and EB is enabled for Q0 and "
                                      "Q1 and sp is disabled for all queues",
                              steps="""
                              1. Create stream with dscp 0 and dscp 1 on port 1 and with dscp 2 and 3 on port 3
                              2. Ensure strict priority is applied to all queues on egress port.
                              3. Start traffic
                              4. After 10 seconds, get RxL1BitRate from spirent for each stream
                              5. Verify bandwidth is such ones with EB enabled hoggs the bandwidth and rest serve in RR 
                              """)


class SP_Shaper_Q0_Cir(SP_Shaper_Q0_SP_EB):
    testcase_streamblocks = {}
    streamblock_handles_list = []
    cir = "within_eb_6"
    test_streams = qos_sp_json[cir]
    ...
    def describe(self):
        self.set_test_details(id=6,
                              summary="To test scheduler when in extra bandwidth zone and SP is enabled for all "
                                      "queues and EB is disabled for all queues. Q0 has cir to 8400, rest have 0",
                              steps="""
                              1. Create stream with dscp 0 and dscp 1 on port 1 and with dscp 2 and 3 on port 3
                              2. Ensure strict priority is applied to all queues on egress port.
                              3. Start traffic
                              4. After 10 seconds, get RxL1BitRate from spirent for each stream
                              5. Verify bandwidth is such that Q0 gets bandwidth as it has cir assgined and remaining 
                                 bandwidth is served in RR fashion to all queues 
                              """)


if __name__ == "__main__":
    ts = SpirentSetup()
    ts.add_test_case(SP_Shaper_Q0_SP_EB())
    ts.add_test_case(EB_SP_Shaper_DWRR_Non_SP())
    ts.add_test_case(SP_Shaper_All_SP_EB())
    ts.add_test_case(SP_Shaper_All_SP_Non_EB())
    ts.add_test_case(SP_Shaper_Q0_Q1_EB())
    ts.add_test_case(SP_Shaper_Q0_Cir())
    ts.run()
...
class Cir_1(FunTestCase):
    ...
    def describe(self):
        self.set_test_details(id=1,
                              ...
                              5. Verify bandwidth is such that two queues get same bandwidth each
                              """)
    ...
    def setup(self):
        ...
        for port, streams in self.test_streams.iteritems():
            ...
            for stream_details in streams:
                ...
                counter += 1
                self.testcase_streamblocks[str(port)][stream_details['dscp']]['streamblock_obj'] = current_streamblock_obj
    ...
    def run(self):
        ...
        add_on_bw = (self.max_egress_load - self.used_bandwidth)/float(self.total_streams)
        ...
        for port, streams in self.test_streams.iteritems():
            for stream_details in streams:
                ...
                result_dict[stream_details['dscp']]['expected'] = expected_load_value + add_on_bw
...
class Cir_2(Cir_1):
    ...
    def describe(self):
        self.set_test_details(id=2,
                              summary="Test shaper cir by having same ingress load such that queues have "
                                      "different cir rate",
                              steps="""
                                    ...
                                    4. Verify bandwidth is such that queues get bandwidth based on cir rates assigned
                                    """)
...
class Cir_3(Cir_1):
    ...
    def describe(self):
        self.set_test_details(id=3,
                              summary="Test shaper cir having different ingress load and different cir rates",
                              steps="""
                              1. Create stream with dscp 8 and dscp 9 on port 1 and with dscp 10 and 11 on port 3
                              2. Start traffic
                              3. After 10 seconds, get RxL1BitRate from spirent for each stream
                              4. Verify bandwidth is based on shaper configured
                              """)


class Shaper_SP(Cir_1):
    testcase_streamblocks = {}
    streamblock_handles_list = []
    pir = "shaper_sp"
    test_streams = qos_sp_json[pir]
    ...
    def describe(self):
        self.set_test_details(id=5,
                              summary="Test shaper and strict priority on queues",
                              steps="""
                              1. Create stream with dscp 0 on port 1 and with dscp 1 and 2 on port 3
                              2. Ensure strict priority is applied to dscp 0 on egress port.
                              3. Start traffic
                              4. After 10 seconds, get RxL1BitRate from spirent for each stream
                              5. Verify bandwidth is based on shaper configured
                              """)
...
if __name__ == "__main__":
    ...
    ts = SpirentSetup()
    ts.add_test_case(Cir_1())
    ts.add_test_case(Cir_2())
    ts.add_test_case(Cir_3())
    #ts.add_test_case(Shaper_SP())
    ts.run()
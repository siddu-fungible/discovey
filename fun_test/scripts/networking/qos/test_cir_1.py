...
class SpirentSetup(FunTestScript):
    ...
    def describe(self):
        self.set_test_details(steps="""
                ...
                5. Subscribe to stream resultrs
    ...
    def setup(self):
        ...
        for port, streams in test_streams.iteritems():
            ...
            for stream_details, dscp_val in zip(streams, current_list):
                self.used_bandwidth += get_load_value_from_load_percent(load_percent=stream_details['cir_bw'],
                ...
                if self.pir:
                    ...
                    fun_test.test_assert(set_rate, "Ensure strict priority and extra bbandwidth is disabled")
                    set_rate = network_controller_obj.set_qos_scheduler_config(port_num=dut_port_2,
                                                                               queue_num=dscp_val,
                                                                               scheduler_type=network_controller_obj.SCHEDULER_TYPE_SHAPER,
                                                                               shaper_enable=True,

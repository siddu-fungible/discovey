...
port_1_priority_list = priority_list[:2]
port_3_priority_list = priority_list[2:]
...
class SpirentSetup(FunTestScript):
    ...
    def setup(self):
        global template_obj, port_1, port_2, pfc_frame, subscribe_results, network_controller_obj, dut_port_2, \
            dut_port_1, hnu, shape, dut_port_3, port_obj_list, port_3
        ...
        # TODO: To be changed when curl calls work
        ...
        for dut_port in dut_port_list[::2]:
            ...
            for key in priority_dict.keys():
                ...
                port_quanta = network_controller_obj.set_priority_flow_control_quanta(port_num=dut_port,
                                                                                      ...
                                                                                      shape=shape)
                fun_test.test_assert(port_quanta, "Ensure quanta %s is set on port %s" % (quanta, dut_port))
        ...
        ports_map['FPG' + str(dut_port_3)] = all_ports_map['FPG' + str(dut_port_3)]
        ...
        fun_test.test_assert(streamblock_2, message="Creating pfc streamblock with priority 4 on port "
                                                    "%s" % port_2)
        ...
        for key, val in priority_dict.iteritems():
            ...
            current_port = port_1
            if val['priority_val'] in port_3_priority_list:
                current_port = port_3
            ...
            streamblock_1 = template_obj.configure_stream_block(create_streamblock_1, port_handle=current_port)
...
class TestCase1(FunTestCase):
    ...
    def run(self):
        ...
        for dut_port in dut_port_list:
            if str(dut_port) == str(dut_port_2):
                continue
            result_dict[psw_port_pg_counters][dut_port] = {}
            result_dict[fpg_cbfcpause_counters_tx][dut_port] = {}
            fun_test.log("Get psw port group enqueue dequeue counters on %s" % dut_port)
            result_dict[psw_port_pg_counters][dut_port] = \
                get_psw_port_enqueue_dequeue_counters(network_controller_obj=network_controller_obj,
                                                      dut_port=dut_port, pg=True, hnu=hnu, priority_list=priority_list)

            fun_test.log("Fetch fpg stats on dut port %s" % dut_port)
            result_dict[fpg_cbfcpause_counters_tx][dut_port] = \
                get_fpg_port_cbfcpause_counters(network_controller_obj=network_controller_obj, dut_port=dut_port,
        ...
        for dut_port in dut_port_list:
            if not dut_port == dut_port_2:
                check_priority_list = port_1_priority_list
                if dut_port == dut_port_3:
                    check_priority_list = port_3_priority_list

                for current_priority in check_priority_list:
                    fun_test.test_assert(not result_dict[psw_port_pg_counters][dut_port][current_priority][dequeue],
                                         message="Ensure pg dequeue is not happening for queue q_%s on port %s" %
                                                 (current_priority, dut_port))

                    fun_test.test_assert(result_dict[psw_port_pg_counters][dut_port][current_priority][enqueue],
                                         message="Ensure pg enqueue is happening for queue q_%s on port %s" %
                                                 (current_priority, dut_port))

                    fun_test.test_assert(result_dict[fpg_cbfcpause_counters_tx][dut_port][current_priority],
                                         message="Ensure CBFCPause_Transmitted in seen for priority %s on port %s" %
                                                 (current_priority, dut_port))

            else:
                for priority_val in priority_list[::differentiator_queue_value]:
                    if not priority_val == 4:
                        fun_test.test_assert(result_dict[psw_port_q_counters][priority_val][dequeue],
                                             message="Ensure q dequeue is happening for queue q_%s on port %s" %
                                                     (priority_val, dut_port))

                        fun_test.test_assert(not result_dict[fpg_cbfcpause_counters_rx][priority_val],
                                             message="Ensure CBFCPause_Received in not seen for priority %s on port %s" %
                                                     (priority_val, dut_port))
                    else:
                        fun_test.test_assert(not result_dict[psw_port_q_counters][priority_val][dequeue],
                                             message="Ensure q dequeue is not happening for queue q_%s on port %s" %
                                                     (priority_val, dut_port))

                        fun_test.test_assert(result_dict[fpg_cbfcpause_counters_rx][priority_val],
                                             message="Ensure CBFCPause_Received in seen for priority %s on port %s" %
                                                     (priority_val, dut_port))

                    fun_test.test_assert(result_dict[psw_port_q_counters][priority_val][enqueue],
                                         message="Ensure q enqueue is happening for queue q_%s on port %s" %
                                                 (priority_val, dut_port))
        ...
        capture_obj = Capture()
        for port in port_obj_list:
            if str(port) == str(port_2):
                continue
            capture_obj_dict[port] = {}
            start_capture_1 = template_obj.stc_manager.start_capture_command(capture_obj=capture_obj,
                                                                             port_handle=port)
            fun_test.test_assert(start_capture_1, "Started capture on port %s" % port)
            capture_obj_dict[port] = capture_obj
        ...
        for key, val in capture_obj_dict.iteritems():
            stop_capture = template_obj.stc_manager.stop_capture_command(val._spirent_handle)
            fun_test.test_assert(stop_capture, "Stopped capture on port %s" % key)
            ...
            saved = template_obj.stc_manager.save_capture_data_command(capture_handle=val._spirent_handle,
            ...
            for priority in check_priority_list:

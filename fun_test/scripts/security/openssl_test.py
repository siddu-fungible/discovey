...
hmac_sha1_path = "/hmac_sha1.txt"
hmac_sha224_path = "/hmac_sha224.txt"
...
class OpenSslScript(FunTestScript):
    ...
    def compute_digests(self, host, openssl_template, path, algorithm):
        ...
        for index, digest_input in enumerate(input_list):
            host.create_file_using_printf("{}{}.bin".format(base_file_name, index), contents=digest_input)
    ...
    def compute_hmacs(self, host, openssl_template, path, algorithm):
        ...
        for index, hmac_input in enumerate(input_list):
            host.create_file_using_printf("{}{}.bin".format(base_file_name, index), contents=(hmac_dict[hmac_input].values())[0])
...
class OpenSslTestCase3(FunTestCase):
    ...
    def run(self):
        openssl_template = fun_test.shared_variables["openssl_template"]
        host = fun_test.shared_variables["host"]
        fun_test.test_assert((openssl_script.compute_hmacs(host, openssl_template, hmac_sha1_path, openssl_template.SHA1)),
                             "sha1 hmacs match")
...
class OpenSslTestCase4(FunTestCase):
    ...
    def run(self):
        openssl_template = fun_test.shared_variables["openssl_template"]
        host = fun_test.shared_variables["host"]
        fun_test.test_assert((openssl_script.compute_hmacs(host, openssl_template, hmac_sha224_path, openssl_template.SHA224)),
                             "sha224 hmacs match")

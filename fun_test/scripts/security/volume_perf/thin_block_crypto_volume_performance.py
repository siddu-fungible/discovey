...
class BLTVolumePerformanceTestcase(FunTestCase):
    ...
    def setup(self):
        ...
        if "blt" not in fun_test.shared_variables or not fun_test.shared_variables["blt"]["setup_created"]:
            ...
            self.xts_xtweak = generate_key(self.xtweak_size)
    ...
    def run(self):
        ...
        for combo in self.fio_bs_iodepth:
            ...
            for mode in self.fio_modes:
                ...
                encryption_status = self.volume_details["encrypt"]
                if encryption_status:
                    ...
                    for key, value in final_volume_status[combo][mode].items():
                        if key == "num_writes":
                            total_vol_write = value
                        elif key == "num_reads":
                            total_vol_read = value

                total_vol_ops = total_vol_read + total_vol_write
                fun_test.test_assert(expression=total_crypto_ops == total_vol_ops,
                                     message="No error in crypto ops.")

...
class LibkcapiScript(FunTestScript):
    def describe(self):
        ...
        1. Login the host with given IP, username & ssh port
        2. Load funcrypto module
        3. Start tests
                              """)

    def setup(self):
        host = Linux(host_ip="192.168.56.5", ssh_username="root", ssh_port="22")
        # Check what should be the prompt terminator. If there is a mismatch script won't proceed.
        # host.prompt_terminator = Linux.NON_ROOT_PROMPT_TERMINATOR_DEFAULT
        # host.ROOT_PROMPT_TERMINATOR_DEFAULT = r'\$'
        # host.set_root_prompt_terminator(r'\$ ')     #
        ...
        for file_paths in gcm_vector_path:
            ...
            for enc_dict in enc_dicts:
                ...
                fun_test.simple_assert((enc_output == enc_dict['result'] or
                                        (enc_output in enc_dict['tag'])), "encryption test")
            ...
            for dec_dict in dec_dicts:
                ...
                fun_test.simple_assert((dec_output == dec_dict['result']) or
                                       ...
                                       (dec_output in "EBADMSG"), "decryption test")
...
class TestCcm(FunTestCase):
    def describe(self):
        self.set_test_details(id=2,
                              ...
                              steps="""
        1. Encrypt and decrypt ccm(aes) with a sample plain text using kcapi command 
        2. Compare with the known output and ensure they are equal
        ...
        for file_paths in ccm_vector_path:
            ...
            for enc_dict in enc_dicts:
                enc_output = (libkcapi_template.kcapi_cmnd(LibkcapiTemplate.CCM_AES, enc_dict['cipher_type'],
                                                           ...
                                                           tag_len=enc_dict['tag_len'])).strip()
                fun_test.simple_assert((enc_output == enc_dict['result']), "encryption test")
            ...
            for dec_dict in dec_dicts:
                dec_output = (libkcapi_template.kcapi_cmnd(LibkcapiTemplate.CCM_AES, dec_dict['cipher_type'],
                                                           ...
                                                           tag=dec_dict['tag'])).strip()
                fun_test.simple_assert((dec_output == dec_dict['result']), "decryption test")
...
class TestSha1(FunTestCase):
    ...
    def run(self):
        ...
        for enc_dict in enc_dicts:
            ...
            fun_test.simple_assert((enc_output == enc_dict['MD']), "digest test")
